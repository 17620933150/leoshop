...
<article class="page-container">
	<form action="{{ url('Admin/course') }}" method="post" class="form form-horizontal" id="form-add-course">
		{{ csrf_field() }}
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>课时简介：名称：</label>
		...
		<div class="row cl">
		 ...
		 <div class="row cl">
	      <label class="form-label col-xs-4 col-sm-3">课时简介：简介：</label>
	      <div class="formControls col-xs-8 col-sm-6">
	    ...
	    <div class="row cl">
	      <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>课时简介：时长(单位：小时)：</label>
	      <div class="formControls col-xs-4 col-sm-6">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-3 col-sm-3">
				<div id="baidu-webuploader-video">选择视频</div>
				<span id="baidu-webuploader-stuart-video" class="btn btn-primarv radius">开始上传</span>
				<input type="hidden" name="cover">
				<!-- 上传进度条 -->
				<div id="processing-video">
					...
					<span id="progress-num-video">0%</span>
				</div>
			</div>
		</div>
...
$(function(){
    ...
    uploader.on('uploadProgress',function(file,percentage) {//percentage 表示上传的百分比进度
    	...
    	$("#progress-num-video").html(percentage * 100 + '%');
    })
    //接收webuploader上传文件以后服务器返回的信息
    uploader.on('uploadSuccess',function(file,msg) {
    	$('#processing-video .sr-only').css('width','100%');
    	$("#progress-num-video").html('100%');
    	$('#'+file.id).addClass('uploader-state-done');
    	//把服务器返回的图片地址保存到隐藏域
    	$('input[name=cover]').val(msg.data);
    })
    //关闭webuploader自动上传,设置为手动上传
    $('#baidu-webuploader-stuart-video').click(function() {
    	//调用webupload插件 开始上传
