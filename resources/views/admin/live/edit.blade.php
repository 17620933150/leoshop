...
@section("title")添加学科@endsection
@section("content")
{{-- 加载webuploader的css --}}
<link rel="stylesheet" href="{{ asset("backend") }}/lib/webuploader/0.1.5/webuploader.css">
<article class="page-container">
	<form action="{{ url("admin/subject/" . $subjectInfo->id ) }}" method="post" class="form form-horizontal" id="form-add-subject">
		{{ csrf_field() }}
		{{ method_field("PUT") }}
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>学科名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" placeholder="" name="subject_name" id="subject" value="{{ $subjectInfo->subject_name }}">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>logo：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="hidden" name="logo" value="{{ $subjectInfo->logo }}">
				<div id="baidu-webuploader-preview" style="margin-bottom: 6px;">
					<img height="100" src="{{ $subjectInfo->logo }}" alt="">
				</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-3 col-sm-3">
				<input type="text" class="input-text" placeholder="" name="sort" id="sort" value="{{ $subjectInfo->sort }}">
			</div>
		</div>
		<div class="row cl">
...
$(function(){
	...
	uploader.on( 'uploadSuccess', function( file, response ) {
		...
		if( response.status){
			...
			$("input[name=logo]").val(response.data); // 保存到隐藏域
		}
	});
	...
	$("#form-add-subject").validate({
		...
		rules:{
			...
			sort:{
				...
				// minlength:6,
				// maxlength:16,
				// equalTo: "#subject" // 设置当前输入框的值必须和指定id的输入框的值一致
			},
		},
